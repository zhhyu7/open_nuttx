...
class Config:
    def __init__(self, outpath, elf, ldscript):
        ...
        if self.elf is None or \
           os.path.exists(self.elf) is False:
            self.elf = None
            return
...
class KASanRegion:
    ...
    def poison(self, dict):
        ...
        nbits = distance % 8
        if debug:
            print("regin: %08x addr: %08x size: %d bits: %d || poison index: %d nbits: %d"
                  % (self.start,
                     dict['beg'],
                     dict['size'],
...
class KASanInfo:
    ...
    def merge_ranges(self, dict):
        if len(self.data_sections) == 0:
            self.data_sections.append([dict['beg'], dict['beg'] + dict['size_with_redzone']])
            return
        start = dict['beg']
...
def main():
    ...
    if section is None:
        handle_error(config,
                     "Please update the link script, section ['%s'] cannot be found" % (KASAN_SECTION))
