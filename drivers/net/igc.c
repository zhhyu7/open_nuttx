...
#define IGC_RX_SPARE           8

#define IGC_TX_QUOTA           IGC_TX_DESC
#define IGC_RX_QUOTA           (IGC_RX_DESC + IGC_RX_SPARE)

/* NOTE: CONFIG_IOB_ALIGNMENT must match system D-CACHE line size */

#if CONFIG_IOB_NBUFFERS < IGC_RX_QUOTA
#  error CONFIG_IOB_NBUFFERS must be > IGC_RX_QUOTA
#endif
...
static void igc_msix_interrupt(FAR struct igc_driver_s *priv)
{
  ...
  if (icr & IGC_IC_LSC)
    {
      if (igc_getreg_mem(priv, IGC_STATUS) & IGC_STATUS_LU)
        {
          ninfo("Link up\n");
