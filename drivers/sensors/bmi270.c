...
static uint8_t bmi270_getreg8(FAR struct bmi270_dev_s *priv,
                              uint8_t regaddr);
static void bmi270_putreg8(FAR struct bmi270_dev_s *priv, uint8_t regaddr,
                           uint8_t regval);

static void bmi270_getregs(FAR struct bmi270_dev_s *priv, uint8_t regaddr,
                           FAR uint8_t *regval, int len);
static void bmi270_putregs(FAR struct bmi270_dev_s *priv, uint8_t regaddr,
                           FAR uint8_t *regval, int len);
...
static int bmi270_checkid(FAR struct bmi270_dev_s *priv);
...
static const uint8_t g_bmi270_config_file[] =
{
...
static inline void bmi270_configspi(FAR struct spi_dev_s *spi)
{
  /* Configure SPI for the BMI270 */
 ...
 *
 * Description:
 *   Read from an 8-bit BMI270 register
 *
 ****************************************************************************/

static uint8_t bmi270_getreg8(FAR struct bmi270_dev_s *priv, uint8_t regaddr)
{
#ifdef CONFIG_SENSORS_BMI270_I2C
 ...
 *
 * Description:
 *   Write a value to an 8-bit BMI270 register
 *
 ****************************************************************************/
                           ...
                           FAR uint8_t *regval, int len)
{
  ...
  SPI_SEND(priv->spi, regaddr);
  SPI_SNDBLOCK(priv->spi, regval, len);
...
/****************************************************************************
 * Name: bmi270_set_normal_imu
 *
 * Description:
 *   set bmi270 to normal IMU mode.
 *
 ****************************************************************************/

static void bmi270_set_normal_imu(FAR struct bmi270_dev_s *priv)
{
  /* Enable acquisition of acc and gyro */

  bmi270_putreg8(priv, BMI270_PWR_CTRL, PWRCTRL_GYR_EN | PWRCTRL_ACC_EN);
  up_mdelay(30);
  ...
  bmi270_putreg8(priv, BMI270_PWR_CONF, PWRCONF_FSW_ON);
}

/****************************************************************************
 * Name: bmi270_init_seq
 *
 * Description:
 *   set bmi270 to normal IMU mode.
 *
...
static int bmi270_init_seq(FAR struct bmi270_dev_s *priv)
{
  ...
  bmi270_putreg8(priv, BMI270_PWR_CONF, 0);
  up_mdelay(5);
 ...
 *
 * Description:
 *   Read and verify the BMI270 chip ID
 *
 ****************************************************************************/
