 ...
 * @cpu: CPU to find the policy for.
 *
 * Call cpufreq_cpu_get() to get a reference on the cpufreq policy for @cpu
 *and
                                   ...
                                   struct cpufreq_freqs *freqs)
{
  ...
  WARN_ON(
    !(cpufreq_driver->flags & CPUFREQ_ASYNC_NOTIFICATION) &&
...
unsigned int cpufreq_policy_transition_delay_us(
...
static ssize_t show_scaling_setspeed(struct cpufreq_policy *policy,
                                     char *buf)
{
...
{
  &cpuinfo_min_freq.attr,            &cpuinfo_max_freq.attr,
  &cpuinfo_transition_latency.attr,  &scaling_min_freq.attr,
  &scaling_max_freq.attr,            &affected_cpus.attr,
  &related_cpus.attr,
  &scaling_governor.attr,            &scaling_driver.attr,
  &scaling_available_governors.attr, &scaling_setspeed.attr,
  NULL
};
...
static void add_cpu_dev_symlink(struct cpufreq_policy *policy,
...
{
  struct cpufreq_policy *policy = container_of(work, struct cpufreq_policy,
                                               update);
...
static int cpufreq_notifier_max(struct notifier_block *nb,
                                unsigned long freq, void *data)
{
  struct cpufreq_policy *policy = container_of(nb, struct cpufreq_policy,
                                               nb_max);
      ...
      if (ret == -EINVAL)
        {
          ...
          BUG_ON(ret);
          pr_info(
            "%s: CPU%d: Running at unlisted initial frequency: "
            "%u KHz, changing to: %u KHz\n",
...
static void __cpufreq_offline(unsigned int cpu,
                              struct cpufreq_policy *policy)
{
  int ret;
...
static void cpufreq_remove_dev(struct device *dev,
 ...
 *
 * Return the name string of the currently registered cpufreq driver or NULL
 *if
 * none.
 */
 ...
 *
 * Return the private data of the currently registered cpufreq driver,
 * or NULL if no cpufreq driver has been registered.
 */
 ...
 * sections and it is expected to select the minimum available frequency
 * greater than or equal to @target_freq (CPUFREQ_RELATION_L).
 *
 * This function must not be called if policy->fast_switch_enabled is unset.
 *
 * Governors calling this function must guarantee that it will never be
 * invoked twice in parallel for the same policy and that it will never be
 * called in parallel with either ->target() or ->target_index() for
 * the same policy.
 *
 ...
 * suitable for being called from within RCU-sched read-side critical
 * sections and it is expected to select a suitable performance level
 * equal to or above
 ...
 *
 * Governors calling this function must guarantee that it will never be
 * invoked twice in parallel for the same CPU and that it will never be
 * called in parallel with either ->target() or ->target_index() or
 * ->fast_switch() for the same CPU.
 */

void cpufreq_driver_adjust_perf(unsigned int cpu, unsigned long min_perf,
                                 ...
                                 struct cpufreq_freqs *freqs, int index)
{
  ...
  pr_debug(
    "%s: cpu: %d, switching to intermediate freq: "
    "oldfreq: %u, intermediate freq: %u\n",
...
static int cpufreq_init_governor(struct cpufreq_policy *policy)
{
  ...
  if (policy->governor->flags & CPUFREQ_GOV_DYNAMIC_SWITCHING &&
      cpufreq_driver->flags & CPUFREQ_NO_AUTO_DYNAMIC_SWITCHING)
    {
      ...
      if (gov)
        {
          pr_warn(
            "Can't use %s governor as dynamic switching is disallowed. "
  ...
  policy->strict_target =
    !!(policy->governor->flags & CPUFREQ_GOV_STRICT_TARGET);
 ...
 *
 * Invoke the cpufreq driver's ->verify() callback to sanity-check the
 *frequency
 * limits to be set for the policy, update @policy with the verified limits
 * values and either invoke the driver's ->setpolicy() callback (if present)
 *or
 * carry out a governor update for @policy.  That is, run the current
 *governor's
 ...
 * cpufreq_set_policy() to re-apply the min and max limits, which triggers
 * the evaluation of policy notifiers and the cpufreq driver's ->verify()
 * callback for the policy in question, among other things.
 */
...
void cpufreq_update_policy(unsigned int cpu)
{
  ...
  if (cpufreq_driver->get && has_target() &&
      (cpufreq_suspended ||
       WARN_ON(!cpufreq_verify_current_freq(policy, false))))
    {
      goto unlock;
    }
...
module_param_string(default_governor, default_governor, CPUFREQ_NAME_LEN,
                    0444);
