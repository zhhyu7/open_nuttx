...
{
  PM_QOS_FLAGS_UNDEFINED = -1, PM_QOS_FLAGS_NONE, PM_QOS_FLAGS_SOME,
  PM_QOS_FLAGS_ALL,
};
...
{
  DEV_PM_QOS_RESUME_LATENCY = 1, DEV_PM_QOS_LATENCY_TOLERANCE,
  DEV_PM_QOS_MIN_FREQUENCY, DEV_PM_QOS_MAX_FREQUENCY, DEV_PM_QOS_FLAGS,
};
...
{
  return IS_ERR_OR_NULL(dev->power.qos) ? PM_QOS_RESUME_LATENCY_NO_CONSTRAINT
...
static inline int dev_pm_qos_expose_latency_limit(struct device *dev,
                                                  s32 value)
{
  return 0;
}
...
static inline int dev_pm_qos_update_flags(struct device *dev,
                                          s32 m, bool set)
{
  return 0;
}
