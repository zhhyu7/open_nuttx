...
{
  CPUFREQ_TABLE_UNSORTED, CPUFREQ_TABLE_SORTED_ASCENDING,
...
/**
 * Used for passing new cpufreq policy data to the cpufreq driver's
 * ->verify() callback for sanitization.
 * That callback is only expected to modify the min
...
static inline void cpufreq_stats_record_transition(
...
#define CPUFREQ_RELATION_L      0       /* lowest frequency at or above target
                                         * */
#define CPUFREQ_RELATION_H      1       /* highest frequency below or at
                                         * target */
#define CPUFREQ_RELATION_C      2       /* closest frequency to target */

/* relation flags */

#define CPUFREQ_RELATION_E      BIT(2)  /* Get if possible an efficient
                                         * frequency */
...
/**
 * Set by drivers that need to update internal upper and lower boundaries
 * along with the target frequency and so the core and governors should also
 * invoke the diver if the target frequency does not change, but the policy
 * min o rmax may have changed.
 */
 ...
 * would be created in cpu/cpu<num>/cpufreq/ directory and so they can use
 * the same governor with different tunables for different clusters.
 */
 ...
 * frequency present in freq-table exposed by the driver. For these drivers
 * if CPU is found running at an out of table freq, we will try to set it
 * to a freq from the table. And if that fails, we will stop further boot
 * process by issuing a BUG_ON().
 */
...
static inline void cpufreq_verify_within_limits(
  struct cpufreq_policy_data *policy, unsigned int min, unsigned int max)
{
  if (policy->min < min)
    {
      policy->min = min;
 ...
 * The ondemand governor will work on any processor with transition latency
 * <= 10ms, using appropriate sampling rate.
 */
...
static inline void cpufreq_policy_apply_limits(struct cpufreq_policy *policy)
{
  if (policy->max < policy->cur)
    {
      __cpufreq_driver_target(policy, policy->max, CPUFREQ_RELATION_HE);
    }
  else if (policy->min > policy->cur)
    {
      __cpufreq_driver_target(policy, policy->min, CPUFREQ_RELATION_LE);
    }
}
                                             ...
                                             bool efficiencies)
{
  ...
  if (policy->freq_table_sorted == CPUFREQ_TABLE_SORTED_ASCENDING)
    {
      return cpufreq_table_find_index_ac(policy, target_freq, efficiencies);
    }
  else
    {
      return cpufreq_table_find_index_dc(policy, target_freq, efficiencies);
    }
}
  ...
  unsigned int relation)
{
  ...
  if (unlikely(policy->freq_table_sorted == CPUFREQ_TABLE_UNSORTED))
    {
      return cpufreq_table_index_unsorted(policy, target_freq, relation);
    }
retry:
  switch (relation)
    {
    case CPUFREQ_RELATION_L:
      idx = cpufreq_table_find_index_l(policy, target_freq, efficiencies);
      break;

    case CPUFREQ_RELATION_H:
      idx = cpufreq_table_find_index_h(policy, target_freq, efficiencies);
      break;

    case CPUFREQ_RELATION_C:
      idx = cpufreq_table_find_index_c(policy, target_freq, efficiencies);
      break;
...
static inline int cpufreq_table_count_valid_entries(
  const struct cpufreq_policy *policy)
{
  ...
  cpufreq_for_each_valid_entry(pos, policy->freq_table) count++;
                                    ...
                                    struct of_phandle_args *args)
{
  ...
  ret = of_parse_phandle_with_args(cpu_np, list_name, cell_name, 0, args);
  if (ret < 0)
    {
      return ret;
    }
                                               ...
                                               struct of_phandle_args *pargs)
{
  ...
  for_each_possible_cpu(cpu)
  {
    ...
    if (pargs->np == args.np && pargs->args_count == args.args_count &&
        !memcmp(pargs->args, args.args,
                sizeof(args.args[0]) * args.args_count))
      {
...
#else
static inline void sched_cpufreq_governor_change(
  struct cpufreq_policy *policy, struct cpufreq_governor *old_gov)
{
}
...
static inline void cpufreq_register_em_with_opp(
                                struct cpufreq_policy *policy)
{
