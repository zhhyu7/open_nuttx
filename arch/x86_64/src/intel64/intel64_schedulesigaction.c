      ...
      if (tcb == this_task_inirq())
        {
           ...
           */
          ...
          else
            {
              ...
              tcb->xcp.saved_rip    = get_current_regs()[REG_RIP];
              tcb->xcp.saved_rsp    = tcb->xcp.regs[REG_RSP];
              tcb->xcp.saved_rflags = get_current_regs()[REG_RFLAGS];

              /* Then set up to vector to the trampoline with interrupts
               * disabled
               */

              get_current_regs()[REG_RIP]    = (uint64_t)x86_64_sigdeliver;
              get_current_regs()[REG_RFLAGS] = 0;
